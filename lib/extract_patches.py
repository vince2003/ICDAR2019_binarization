...
def get_datasets(imgs_dir,groundTruth_dir,borderMasks_dir,train_test="null"):
    Nimgs=len(list(paths.list_images(imgs_dir)))
    imgs = np.empty((Nimgs,height,width,channels))
    groundTruth = np.empty((Nimgs,height,width))
    border_masks = np.empty((Nimgs,height,width))
    
    
#    dataPaths_raw=natsorted(list(paths.list_images(imgs_dir)))
#    test_lst_raw=[dataPath.strip()for dataPath in dataPaths_raw]
    
    test_lst_raw = []
    for (dirpath, dirnames, filenames) in walk(imgs_dir):
        test_lst_raw.extend(filenames)
        break
    test_lst_raw=natsorted(test_lst_raw)
    
#    dataPaths_gt=natsorted(list(paths.list_images(groundTruth_dir)))
#    test_lst_gt=[dataPath.strip()for dataPath in dataPaths_gt]
    
    test_lst_gt = []
    for (dirpath, dirnames, filenames) in walk(groundTruth_dir):
        test_lst_gt.extend(filenames)
        break
    test_lst_gt=natsorted(test_lst_gt)

    
#    pdb.set_trace()
    
#    dataPaths_mask=natsorted(list(paths.list_images(borderMasks_dir)))
#    test_lst_mask=[dataPath.strip()for dataPath in dataPaths_mask]
    
    test_lst_mask = []
    for (dirpath, dirnames, filenames) in walk(borderMasks_dir):
        test_lst_mask.extend(filenames)
        break
    test_lst_mask=natsorted(test_lst_mask)
    
    #pdb.set_trace()   
    
    
    for idx in range(0, len(test_lst_raw)):        
                    #original
        print("original image: " +test_lst_raw[idx])
        test_lst_raw[idx]=imgs_dir+test_lst_raw[idx]
        img = Image.open(test_lst_raw[idx])
        imgs[idx] = np.asarray(img)
        #corresponding ground truth
        #groundTruth_name = files[i][0:2] + "_manual1.gif"
        #pdb.set_trace()
        print("ground truth name: " + test_lst_gt[idx])
        test_lst_gt[idx]=groundTruth_dir+test_lst_gt[idx]
        g_truth = Image.open(test_lst_gt[idx])
        #g_truth=255*g_truth  #  ------------------them vao
        groundTruth[idx] = np.asarray(g_truth)*255 # chuyen qua array
        #corresponding border masks
        #pdb.set_trace()
        
        
        border_masks[idx] = np.zeros([height,width],dtype=np.uint8)
        border_masks[idx].fill(255) # or img[:] = 255
    ...
    assert(np.min(groundTruth)==0 and np.min(border_masks)==255)
...
def get_data_test_direct(DRIVE_test_imgs_original, DRIVE_test_groudTruth,DRIVE_test_borderMasks, Imgs_to_test, patch_height, patch_width, stride_height, stride_width):
    ### test
    test_imgs_original,test_masks,test_border_masks = get_datasets(DRIVE_test_imgs_original,DRIVE_test_groudTruth,DRIVE_test_borderMasks,train_test="null")
    ...
    return patches_imgs_test, test_imgs.shape[2], test_imgs.shape[3], test_masks,test_border_masks
