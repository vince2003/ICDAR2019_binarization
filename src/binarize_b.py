...
ap.add_argument("--result","--arg2",type=str,default="./result_b/")
...
check_path = 'LadderNetv65_layer_%d_filter_%d.pt7'% (3,16) #'UNet16.pt7'#'UNet_Resnet101.pt7'



net = LadderNetv6(num_classes=2,layers=3,filters=16,inplanes=1)
...
#    w_win=h_img
    ...
    #pdb.set_trace()
    patch=np.zeros((h_win, w_win, 3), dtype=np.uint8)
    ...
    if ((w_win <= width) and (h_win <= height)):
        ...
        for r in range(0, height, h_win):
            for c in range(0, width, w_win):
                ...
                if(len(in_.shape)==2):
                    patch=np.zeros((h_img, w_img), dtype=np.uint8)
                ...
                if(len(in_.shape)==3):
                    patch=np.zeros((h_img, w_img, 3), dtype=np.uint8)
                    patch[:,:,0] = in_[r:b_b, c:r_b,0]
                    patch[:,:,1] = in_[r:b_b, c:r_b,1]
                    patch[:,:,2] = in_[r:b_b, c:r_b,2]
                ...
                #ret,im_thresh0 = cv2.threshold(final,170,255,cv2.THRESH_BINARY)
                
                
                #imsave(path_experiment+"net1_"+ test_lst[idx][pos[-1]+1:],255-im_thresh0)
                    
    multi_scale = []
                    
    multi_scale.append(final)
    ...
    if ((w_win <= width) and (h_win <= height)):
        ...
        for r in range(0, height, h_win):
            for c in range(0, width, w_win):
                ...
                #imsave(path_experiment +"net2_"+ test_lst[idx][pos[-1]+1:],255-im_thresh1)
                    
    multi_scale.append(final)
    multi_scale = np.min(multi_scale, axis = 0) 
    ret,im_thresh2 = cv2.threshold(multi_scale,170,255,cv2.THRESH_BINARY)
    
    pos = [i for i, ltr in enumerate(test_lst[idx]) if ltr == '/']
    print (test_lst[idx][pos[-1]+1:])
    imsave(path_experiment + test_lst[idx][pos[-1]+1:],255-im_thresh2)
